...
terminate:
	printf("\nSimulator terminated at time %f after sending %d messages from "
				 "layer 5\n",
				 time, nsim);
}
...
void init(int argc, char **argv) /* Initialize the simulator */
{
	...
	if (argc != 6) {
		printf(
				"Usage: %s  num_sim  prob_loss  prob_corrupt  interval  debug_level\n",
				argv[0]);
		exit(1);
	}
...
void starttimer(int AorB /* A or B is trying to stop timer */,
								float increment) {
	struct event *q;
	struct event *evptr;
...
/************************** TOLAYER3 ***************/
void tolayer3(int AorB /* A or B is trying to stop timer */,
							struct pkt packet) {
	struct pkt *mypktptr;
...
void tolayer5(int AorB, char datasent[20]) {
	...
	if (lastmsg + 1 != datasent[0] && TRACE == -1)
		printf("Packet " YELLOW "%c" END " recieved at B in " RED
					 "incorrect order" END ", expected " YELLOW "%c\n" END,
					 datasent[0], lastmsg + 1);
	else
		printf("Packet " YELLOW "%c" END " recieved at B in " GREEN
					 "correct order" END "\n",
					 datasent[0]);
	lastmsg++;
}
